...
from time_model import CriticNet
from time_model import Actor
from time_model import ActorCriticModel
from time_model import ReplayMemory
    ...
    if h_status == 0:  #ヒーターOFFの場合
        while True:  #interval毎に閾値選択をして室温が閾値より小さくなるまで繰り返し
            ...
            if action[1] >= next_in_temp:  #ON閾値以下なら
                ...
                mean, std, j, _, _ = agent.update_parameters(memory, 1, j, step_interval, 1)  #パラメータの更新
                step_interval = 0
        ...
        while True:  #interval毎に閾値選択をして室温が閾値より大きくなるまで繰り返し
            ...
            if action[0] <= next_in_temp:  #OFF閾値以上なら
                ...
                reward = environment.reward(state, next_state, pre_time, time)  #状態，次状態，時間間隔から報酬
                memory.push(state=f_state, action=action, reward=reward, next_state=f_next_state, norm_action=norm_action)  #メモリに追加
                mean, std, j, _, _ = agent.update_parameters(memory, 1, j, step_interval, 0)  #パラメータの更新
                step_interval = 0
                f_state = f_next_state
                state = next_state
                pre_time = time
