...
class Actor():
    ...
    def sample(self, mean, std):  #平均と対数偏差から正規分布を作成し，行動と平均（行動）を出力
        ...
        d = mean[0][0].item() - h
        while not done:
            action = normal.rsample()  #正規分布からサンプリング
            if mean[0][0].item()+d > action[0][0].item() and mean[0][0].item()-d < action[0][0].item() and mean[0][1].item()+d > action[0][1].item() and mean[0][1].item()-d < action[0][1].item():
                done = True
...
class ActorCriticModel(object):
    ...
    def update_parameters(self, memory, batch_size, j, step_interval, i):  #パラメータを更新し，損失関数を返す
        ...
        j = j + 0.1*actor_loss.item()/step_interval
